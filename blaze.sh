...
#
make -j100 -C $PWD O=outdir ARCH=arm
...
cd iykonzBlaze
    ...
    echo -e "***********************************************$nocol"
    echo ""
    curl --upload-file outdir/iykonzBlaze/out_done/devOreo_woods*.zip https://transfer.sh/devOreo_woods_8.0.0_Oreo_$BUILD_START.zip
    echo ""
    echo ""
    BUILD_END=$(date +"%s")
